...
from snova.config.common_site_config import update_config, put_config

# imports - third party imports
import click


@click.group(help="Change snova configuration")
def config():
	pass
...
@click.command(
	"dns_multitenant", help="Enable/Disable snova multitenancy on running snova update"
)
@click.argument("state", type=click.Choice(["on", "off"]))
...
def set_common_config(configs):
	...
	update_config(common_site_config, snova_path=".")


@click.command(
	"remove-common-config", help="Remove specific keys from current snova's common config"
)
@click.argument("keys", nargs=-1)
def remove_common_config(keys):
	from snova.snova import Snova

	common_site_config = Snova(".").conf
	for key in keys:
