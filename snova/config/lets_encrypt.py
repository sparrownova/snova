...
from snova.config.production_setup import service
from snova.config.site_config import get_domains, remove_domain, update_site_config
from snova.snova import Snova
from snova.utils import exec_cmd, which
from snova.utils.snova import update_common_site_config
from snova.exceptions import CommandFailedError


def setup_letsencrypt(site, custom_domain, snova_path, interactive):

	site_path = os.path.join(snova_path, "sites", site, "site_config.json")
	...
	if not Snova(snova_path).conf.get("dns_multitenant"):
		print("You cannot setup SSL without DNS Multitenancy")
		return

	create_config(site, custom_domain)
	run_certbot_and_setup_ssl(site, custom_domain, snova_path, interactive)
	setup_crontab()
...
def run_certbot_and_setup_ssl(site, custom_domain, snova_path, interactive=True):
	service("nginx", "stop")
	...
	if custom_domain:
		remove_domain(site, custom_domain, snova_path)
		domains = get_domains(site, snova_path)
		ssl_config["domain"] = custom_domain
		domains.append(ssl_config)
		update_site_config(site, {"domains": domains}, snova_path=snova_path)
	else:
		update_site_config(site, ssl_config, snova_path=snova_path)

	make_nginx_conf(snova_path)
	service("nginx", "start")
...
def setup_wildcard_ssl(domain, email, snova_path, exclude_base_domain):
