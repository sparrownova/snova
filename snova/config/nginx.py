...
from snova.utils import get_snova_name


def make_nginx_conf(snova_path, yes=False, logging=None, log_format=None):
	conf_path = os.path.join(snova_path, "config", "nginx.conf")
	...
	if allow_rate_limiting:
		template_vars.update(
			{
				"snova_name_hash": hashlib.sha256(snova_name).hexdigest()[:16],
...
def make_snova_manager_nginx_conf(snova_path, yes=False, port=23624, domain=None):
	from snova.config.site_config import get_site_config

	template = snova.config.env().get_template("snova_manager_nginx.conf")
	snova_path = os.path.abspath(snova_path)
	sites_path = os.path.join(snova_path, "sites")

	config = Snova(snova_path).conf
	site_config = get_site_config(domain, snova_path=snova_path)
	snova_name = get_snova_name(snova_path)

	template_vars = {
		"port": port,
		"domain": domain,
		"snova_manager_site_name": "snova-manager.local",
		"sites_path": sites_path,
		"http_timeout": config.get("http_timeout"),
		"webserver_port": config.get("webserver_port"),
		"socketio_port": config.get("socketio_port"),
	...
	snova_manager_nginx_conf = template.render(**template_vars)

	conf_path = os.path.join(snova_path, "config", "nginx.conf")

	if not yes and os.path.exists(conf_path):
		click.confirm(
			"nginx.conf already exists and snova-manager configuration will be appended to it. Do you want to continue?",
			abort=True,
		)

	with open(conf_path, "a") as myfile:
		myfile.write(snova_manager_nginx_conf)


def prepare_sites(config, snova_path):
	sites = {
		"that_use_port": [],
		"that_use_dns": [],
		"that_use_ssl": [],
	...
	sites_configs = get_sites_with_config(snova_path=snova_path)
...
def get_sites_with_config(snova_path):
	from snova.snova import Snova
	from snova.config.site_config import get_site_config
	...
	for site in sites:
		try:
			site_config = get_site_config(site, snova_path=snova_path)
	...
	use_wildcard_certificate(snova_path, ret)

	return ret


def use_wildcard_certificate(snova_path, ret):
	"""
	stored in common_site_config.json as:
	"wildcard": {
	        "domain": "*.shopper.com",
	        "ssl_certificate": "/path/to/shopper.com.cert",
	        "ssl_certificate_key": "/path/to/shopper.com.key"
	}
	"""
	from snova.snova import Snova
...
def get_error_pages():
	snova_app_path = os.path.abspath(snova.__path__[0])
	templates = os.path.join(snova_app_path, "config", "templates")
