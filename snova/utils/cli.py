...
def print_snova_version(ctx, param, value):
	"""Prints current snova version"""
	if not value or ctx.resilient_parsing:
		return

	import snova

	click.echo(snova.VERSION)
	ctx.exit()
...
def use_experimental_feature(ctx, param, value):
	...
	if value == "dynamic-feed":
		...
		snova.cli.dynamic_feed = True
		snova.cli.verbose = True
	else:
		from snova.exceptions import FeatureDoesNotExistError

		raise FeatureDoesNotExistError(f"Feature {value} does not exist")

	from snova.cli import is_envvar_warn_set

	if is_envvar_warn_set:
		return

	click.secho(
		"WARNING: snova is using it's new CLI rendering engine. This behaviour has"
