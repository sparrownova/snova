...
def change_dir():
	if os.path.exists("config.json") or "init" in sys.argv:
		return
	dir_path_file = "/etc/sparrow_bench_dir"
...
def change_uid():
	if is_root() and not cmd_requires_root():
		sparrow_user = bench_config.get("sparrow_user")
		ifsparrow_user:
...
def app_cmd(bench_path="."):
	...
	os.execv(f, [f] + ["-m", "sparrow.utils.bench_helper"] + sys.argv[1:])


defsparrow_cmd(bench_path="."):
	f = get_env_cmd("python", bench_path=bench_path)
	os.chdir(os.path.join(bench_path, "sites"))
	os.execv(f, [f] + ["-m", "sparrow.utils.bench_helper", "sparrow"] + sys.argv[1:])
...
def get_sparrow_help(bench_path="."):
	python = get_env_cmd("python", bench_path=bench_path)
	sites_path = os.path.join(bench_path, "sites")
	try:
		out = get_cmd_output(
			f"{python} -msparrow.utils.bench_helper get-sparrow-help", cwd=sites_path
		)
