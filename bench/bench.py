...
class Bench(Base, Validator):
	...
	def build(self):
		# build assets & stuff
		run_sparrow_cmd("build", bench_path=self.name)
...
class BenchApps(MutableSequence):
	...
	def update_apps_states(
		...
		if self.apps and not os.path.exists(self.states_path):
			...
			# Keepingsparrow as the first app.
	...
	def initialize_apps(self):
		try:
			self.apps = [
				...
				if is_sparrow_app(os.path.join(self.bench.name, "apps", x))
			]
			self.apps.remove("sparrow")
			self.apps.insert(0, "sparrow")
		except FileNotFoundError:
...
class BenchSetup(Base):
	...
	def env(self, python="python3"):
		...
		- upgrade env pip
		- installsparrow python dependencies
		"""
		import bench.cli
		import click
		...
		sparrow = os.path.join(self.bench.name, "apps", "sparrow")
		...
		if os.path.exists(sparrow):
			self.run(
				f"{self.bench.python} -m pip install {quiet_flag} --upgrade -e {sparrow}",
				cwd=self.bench.name,
			)
	...
	def backups(self):
		...
		user = self.bench.conf.get("sparrow_user")
