...
class AppMeta:
	def __init__(self, name: str, branch: str = None, to_clone: bool = True):
		...
		class Healthcare(AppConfig):
		        dependencies = [{"sparrow/erpnext": "~13.17.0"}]
		"""
...
def get_app(
	...
	"""bench get-app clones aSparrow App from remote (GitHub or any other git server),
	...
	if resolve_deps:
		...
		if "sparrow" in resolution:
			# Todo: Makesparrow a terminal dependency for allsparrow apps.
...
def new_app(app, no_git=None, bench_path="."):
	if bench.Sparrow_VERSION in (0, None):
		raise NotInBenchDirectoryError(
			f"{os.path.realpath(bench_path)} is not a valid bench directory."
		)
...
def use_rq(bench_path):
	...
	celery_app = os.path.join(bench_path, "apps", "sparrow", "sparrow", "celery_app.py")
