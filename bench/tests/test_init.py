...
from bench.tests.test_base importSPARROW_BRANCH, TestBenchBase
from bench.bench import Bench


# changed fromsparrow_theme because it wasn't maintained and incompatible,
# chat app & wiki was breaking too. hopefullysparrow_docs will be maintained
...
class TestBenchInit(TestBenchBase):
	...
	def test_init(self, bench_name="test-bench", **kwargs):
		...
		app = App("sparrow", bench=test_bench)
	...
	def test_get_app(self):
		...
		exec_cmd(f"bench get-app {TEST_Sparrow_APP} --skip-assets", cwd=bench_path)
		self.assertTrue(os.path.exists(os.path.join(bench_path, "apps", TEST_Sparrow_APP)))
	...
	def test_get_app_resolve_deps(self):
		...
		self.init_bench("test-bench", skip_assets=True)
		bench_path = os.path.join(self.benches_path, "test-bench")
		exec_cmd(f"bench get-app {Sparrow_APP} --resolve-deps --skip-assets", cwd=bench_path)
		self.assertTrue(os.path.exists(os.path.join(bench_path, "apps",SPARROW_APP)))
	...
	def test_install_app(self):
		...
		self.init_bench(bench_name, skip_assets=True)
		exec_cmd(
			f"bench get-app {TEST_Sparrow_APP} --branch master --skip-assets", cwd=bench_path
		)
		...
		installed_app = not exec_cmd(
			f"bench --site {site_name} install-app {TEST_Sparrow_APP}",
			cwd=bench_path,
			_raise=False,
		)
	...
	def test_remove_app(self):
		...
		exec_cmd(
			f"bench get-app {TEST_Sparrow_APP} --branch master --overwrite --skip-assets",
			cwd=bench_path,
		)
		exec_cmd(f"bench remove-app {TEST_Sparrow_APP}", cwd=bench_path)
		...
		self.assertFalse(os.path.exists(os.path.join(bench_path, "apps", TEST_Sparrow_APP)))

	def test_switch_to_branch(self):
		self.init_bench("test-bench", skip_assets=True)
		bench_path = os.path.join(self.benches_path, "test-bench")
		app_path = os.path.join(bench_path, "apps", "sparrow")

		# * chore: change to 14 when avalible
		prevoius_branch = "version-13"
		ifSPARROW_BRANCH != "develop":
			# assuming we follow `version-#`
			prevoius_branch = f"version-{int(Sparrow_BRANCH.split('-')[1]) - 1}"

		successful_switch = not exec_cmd(
			f"bench switch-to-branch {prevoius_branch}sparrow --upgrade",
			cwd=bench_path,
			_raise=False,
		)
		...
		successful_switch = not exec_cmd(
			f"bench switch-to-branch {Sparrow_BRANCH}sparrow --upgrade",
			cwd=bench_path,
			_raise=False,
		)
		if successful_switch:
