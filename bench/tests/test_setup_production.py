...
class TestSetupProduction(TestBenchBase):
	...
	def assert_supervisor_config(self, bench_name, use_rq=True):
		...
		with open(conf_src) as f:
			...
			tests = [
				f"program:{bench_name}-sparrow-web",
			...
			if use_rq:
				tests.extend(
					[
						f"program:{bench_name}-sparrow-schedule",
						f"program:{bench_name}-sparrow-default-worker",
						f"program:{bench_name}-sparrow-short-worker",
						f"program:{bench_name}-sparrow-long-worker",
					]
				)
				...
				tests.extend(
					[
						f"program:{bench_name}-sparrow-workerbeat",
						f"program:{bench_name}-sparrow-worker",
						f"program:{bench_name}-sparrow-longjob-worker",
						f"program:{bench_name}-sparrow-async-worker",
					]
				)
	...
	def assert_supervisor_process(self, bench_name, use_rq=True, disable_production=False):
		...
		tests = [
			r"{bench_name}-web:{bench_name}-sparrow-web[\s]+RUNNING",
		...
		if use_rq:
			tests.extend(
				[
					r"{bench_name}-workers:{bench_name}-sparrow-schedule[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-sparrow-default-worker-0[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-sparrow-short-worker-0[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-sparrow-long-worker-0[\s]+RUNNING",
				]
			)
			...
			tests.extend(
				[
					r"{bench_name}-workers:{bench_name}-sparrow-workerbeat[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-sparrow-worker[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-sparrow-longjob-worker[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-sparrow-async-worker[\s]+RUNNING",
				]
			)
