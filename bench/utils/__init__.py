...
def is_valid_sparrow_branch(sparrow_path: str,sparrow_branch: str):
	...
	:paramsparrow_path: git url
	:typesparrow_path: str
	:paramsparrow_branch: branch to check
	:typesparrow_branch: str
	...
	ifsparrow_branch:
		try:
			...
			raise InvalidRemoteException(f"Invalidsparrow path: {sparrow_path}") from e
...
def check_latest_version():
	...
	try:
		pypi_request = requests.get("https://pypi.org/pypi/sparrow-bench/json")
	except Exception:
...
def run_sparrow_cmd(*args, **kwargs):
	...
	p = subprocess.Popen(
		(f, "-m", "sparrow.utils.bench_helper", "sparrow") + args,
		cwd=sites_dir,
		stdout=stdout,
		stderr=stderr,
	)
...
def get_env_sparrow_commands(bench_path=".") -> List:
	"""Caches all available commands (even custom apps) viaSparrow
	...
	try:
		return json.loads(
			get_cmd_output(
				f"{python} -msparrow.utils.bench_helper get-sparrow-commands", cwd=sites_path
			)
		)
...
def find_org(org_repo):
	...
	raise InvalidRemoteException(f"{org_repo} not found insparrow or erpnext")
...
class _dict(dict):
	...
	# bench port ofsparrow._dict
	def __getattr__(self, key):
		ret = self.get(key)
		# "__deepcopy__" exception added to fixsparrow#14833 via DFP
...
def get_cmd_from_sysargv():
	...
	For Command: `bench --profile --sitesparrowframework.com migrate --no-backup`
	sys.argv: ["/home/sparrow/.local/bin/bench", "--profile", "--site", "sparrownova.com", "migrate", "--no-backup"]
	Actual command run: migrate

	"""
	# context is passed as options tosparrow's bench_helper
	from bench.bench import Bench
	...
	for arg in sys_argv:
		...
		if arg insparrow_context.flags:
			continue

		elif arg insparrow_context.params:
			skip_next = True
			continue
