...
def setup_sudoers(user):
	...
	template = bench.config.env().get_template("sparrow_sudoers")
...
def migrate_site(site, bench_path="."):
	run_sparrow_cmd("--site", site, "migrate", bench_path=bench_path)


def backup_site(site, bench_path="."):
	run_sparrow_cmd("--site", site, "backup", bench_path=bench_path)
...
def fix_prod_setup_perms(bench_path=".",sparrow_user=None):
	from glob import glob
	from bench.bench import Bench

	sparrow_user =sparrow_user or Bench(bench_path).conf.get("sparrow_user")
